...
func newResolveCmd(clientFn func() (v1connect.PaprikaServiceClient, error), output *string) *cobra.Command {
	var file string
	cmd := &cobra.Command{
		Use:   "resolve",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := client.ResolveSource(context.Background(), connect.NewRequest(&paprikav1.ResolveSourceRequest{
				Type:     sourceType,
				SpecJson: specJSON,
			}))
			if err != nil {
