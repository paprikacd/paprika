...
export function ApplicationHealth({ application, release, resources, observedAt, releaseLoading, onSelectResource }: {
  ...
  return <div className="flex min-w-0 flex-col gap-4" aria-label="Application health details">
    <div className="flex flex-wrap items-baseline justify-between gap-2">
      <div>
        <h2 className="font-cond text-board font-semibold">Health evidence</h2>
        <p className="mt-1 text-note text-muted-foreground">Workload observations, configured checks and the current release’s verification.</p>
      </div>
    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-2">
        ...
        {checkNames.length === 0 ? <Empty>No application-level CEL or HTTP checks are configured or reported. Resource readiness and release verification are shown separately below.</Empty> :
          <ul className="divide-y divide-rule-soft">
            {checkNames.map((name) => {
              ...
              return <li key={name} className="space-y-2 px-3.5 py-3">
                <div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-mono text-note">{name}</h3><StatusPill tone={resultTone(check?.status ?? "")} label={check?.status || "Not evaluated"} /></div>
      ...
      <Blueprint>
        <BoardHeader title="Release verification" meta={release?.name} />
    ...
    <Blueprint>
      <BoardHeader title="Resource health" meta={`${healthy}/${health.length} reported healthy`} actions={<button type="button" aria-pressed={attentionOnly} onClick={() => setAttentionOnly(!attentionOnly)} className="cursor-pointer border border-rule px-2 py-1 text-meta">{attentionOnly ? "Show all resources" : "Needs attention"}</button>} />
