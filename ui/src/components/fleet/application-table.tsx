    ...
    >
      <span role="cell" aria-colindex={colIndex.get("application")} className="min-w-0">
        <span className="flex items-center gap-1.75">
          ...
          {identity ? <Link href={applicationURL(identity)} onClick={(event) => event.stopPropagation()} className="truncate font-cond text-name font-semibold tracking-[0.02em] hover:underline">{identity.name}</Link> : <span>Unnamed application</span>}
        </span>
