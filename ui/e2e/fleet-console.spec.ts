...
test("inspects retained dependency failures after recovery", async ({ page }) => {
  ...
  const dependencies = page.getByLabel("Dependency check results")
  ...
  await page.getByText("Captured response", { exact: true }).click()
  await expect(page.locator("pre").filter({hasText:'"status": "degraded"'})).toBeVisible()
})
