...
func (r *Router) Type() string { return ProviderGatewayAPI }
...
func (r *Router) SetWeight(ctx context.Context, weight int32) error {
	...
	if rules == nil {
		return fmt.Errorf("http route %s has no rules", routeName)
	}
...
func (r *Router) RemoveCanary(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("get HTTP route for canary removal: %w", err)
	}
	if rules == nil {
		return nil
	}
	...
	for _, rule := range rules {
		...
		for _, be := range backends {
			...
			name, _, err := unstructured.NestedString(b, "name")
			if err != nil {
				return fmt.Errorf("failed to read backendRef name: %w", err)
			}
			if name == r.canarySvc {
				continue
			}
...
func (r *Router) getHTTPRouteWithRules(ctx context.Context) (string, *unstructured.Unstructured, []any, error) { //nolint:gocritic // bare returns on named results would shadow err
	...
	if err != nil {
		return "", nil, nil, fmt.Errorf("failed to read rules from HTTPRoute %s: %w", routeName, err)
	}
