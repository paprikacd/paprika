...
func (s *S3Source) downloadObject(ctx context.Context, client *s3.Client, localFile string) error {
	...
	if err != nil {
		return errors.Join(
			fmt.Errorf("get object s3://%s/%s: %w", s.Bucket, s.Key, err),
			closeAndRemoveTmp(f, tmpFile),
		)
	}
	defer func() { _ = getOut.Body.Close() }() //nolint:errcheck // best-effort body close

	if _, copyErr := io.Copy(f, getOut.Body); copyErr != nil {
		return errors.Join(
			fmt.Errorf("download object: %w", copyErr),
			closeAndRemoveTmp(f, tmpFile),
		)
	}
	...
	if strings.HasSuffix(s.Key, ".tgz") || strings.HasSuffix(s.Key, ".tar.gz") {
		if tarErr := untarArchive(ctx, tmpFile, filepath.Dir(localFile)); tarErr != nil {
			return errors.Join(
				fmt.Errorf("extract chart archive: %w", tarErr),
				removeTmp(tmpFile),
			)
		}
		if rmErr := removeTmp(tmpFile); rmErr != nil {
			return rmErr
		}
	} else {
...
func closeAndRemoveTmp(f *os.File, tmpFile string) error {
	var errs []error
	if closeErr := f.Close(); closeErr != nil {
		errs = append(errs, fmt.Errorf("close temp file: %w", closeErr))
	}
	if rmErr := os.Remove(tmpFile); rmErr != nil {
		errs = append(errs, fmt.Errorf("remove temp file: %w", rmErr))
	}
	return errors.Join(errs...)
}

func removeTmp(tmpFile string) error {
	if rmErr := os.Remove(tmpFile); rmErr != nil {
