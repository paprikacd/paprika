...
// TemplateRenderer renders Helm charts and resolves sources for templates.
type TemplateRenderer struct {
	WorkDir string
}

// NewTemplateRenderer creates a new TemplateRenderer with the given working directory.
...
func (r *TemplateRenderer) ResolveSource(ctx context.Context, tmpl *paprika.Template) (*source.ResolveResult, error) {
...
func (r *TemplateRenderer) Render(ctx context.Context, tmpl *paprika.Template, params map[string]string) ([]byte, error) {
	...
	if err := templateCmd.Run(); err != nil {
		return nil, fmt.Errorf("helm template failed; stderr: %s: %w", stderr.String(), err)
	}

	return stdout.Bytes(), nil
}

func (r *TemplateRenderer) resolveChartPath(ctx context.Context, tmpl *paprika.Template) (paprika.ChartRef, string, error) {
...
//nolint:cyclop // values file writing has several small branches.
func (r *TemplateRenderer) writeValuesFile(params map[string]string, baseContent string) (string, error) {
	...
	defer f.Close() //nolint:errcheck // safe to ignore close error
	...
	if baseContent != "" {
		...
		if len(params) > 0 && !strings.HasSuffix(baseContent, "\n") {
			if _, err := f.WriteString("\n"); err != nil {
				return "", fmt.Errorf("write values newline: %w", err)
			}
		}
	}
...
func (r *TemplateRenderer) resolveChartFromRepo(ctx context.Context, chart paprika.ChartRef) ([]string, error) {
	...
	if out, addErr := addCmd.CombinedOutput(); addErr != nil {
		return nil, fmt.Errorf("helm repo add failed; output: %s: %w", string(out), addErr)
	}
	...
	if out, updateErr := updateCmd.CombinedOutput(); updateErr != nil {
		return nil, fmt.Errorf("helm repo update failed; output: %s: %w", string(out), updateErr)
	}
