...
func (s *PaprikaServer) ApplyBundle(
	...
	if req.Msg.DryRun {
		app := s.buildApplication(appName, namespace, "")
		rel := s.buildRelease(appName, namespace, "", bundle, evResult.Results)
		return connect.NewResponse(&paprikav1.ApplyBundleResponse{
	...
	app, release, err := s.applyInline(ctx, appName, namespace, bundle, evResult.Results)
	if err != nil {
...
func (s *PaprikaServer) applyInline(
	...
	app, err := s.createOrUpdateApplication(ctx, appName, namespace, snapshotName)
	if err != nil {
		return nil, nil, err
	}

	if err := s.ensureStage(ctx, appName, namespace, releaseName, stageName); err != nil {
		return nil, nil, err
	}

	release := s.buildRelease(appName, namespace, snapshotName, bundle, policyResults)
	...
	if err := s.createSnapshot(ctx, release, appName, namespace, snapshotName, releaseName, bundle); err != nil {
...
func (s *PaprikaServer) createOrUpdateApplication(
	ctx context.Context,
	appName, namespace, snapshotName string,
) (*pipelinesv1alpha1.Application, error) {
	app := s.buildApplication(appName, namespace, snapshotName)
...
func (s *PaprikaServer) ensureStage(
	ctx context.Context,
	appName, namespace, releaseName, stageName string,
) error {
	stage := &pipelinesv1alpha1.Stage{
		ObjectMeta: metav1.ObjectMeta{
			Name:      stageName,
			Namespace: namespace,
			Labels:    s.baseLabels(appName, releaseName),
		},
	...
	if err := s.Create(ctx, stage); err != nil && !apierrors.IsAlreadyExists(err) {
...
func (s *PaprikaServer) createSnapshot(
	...
	appName, namespace, snapshotName, releaseName string,
	bundle []byte,
) error {
	cm := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
...
func (s *PaprikaServer) buildApplication(appName, namespace, snapshotName string) *pipelinesv1alpha1.Application {
...
func (s *PaprikaServer) baseLabels(appName, releaseName string) map[string]string {
